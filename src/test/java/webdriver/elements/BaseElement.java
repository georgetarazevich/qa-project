...
public abstract class BaseElement extends BaseEntity {
	...
	public void waitForIsElementPresent() {
		...
		Assert.assertTrue(formatLogMsg(getLoc("loc.is.absent")),
				element.isDisplayed());
	}
	...
	public void click() {
		...
		browser.getDriver().executeScript(
	...
	public boolean isPresent(int timeout) {
		...
		browser.getDriver().manage().timeouts()
				.implicitlyWait(TIMEOUT_WAIT_0, TimeUnit.SECONDS);
		try {
			wait.until(driver -> {
				try {
		...
		try {
			browser.getDriver()
					...
					.timeouts()
					.implicitlyWait(
							Integer.valueOf(browser.getTimeoutForCondition()),
							TimeUnit.SECONDS);
			return element.isDisplayed();
		} catch (Exception e) {
			warn(e.getMessage());
	...
	public void mouseMoveToElement() {
		waitForIsElementPresent();
		Locatable hoverItem = (Locatable) (browser.getDriver())
				.findElement(locator);
		(browser.getDriver()).getMouse().mouseMove(hoverItem.getCoordinates());
	}
	...
	public void clickByMouse() {
		...
		info(getLoc("loc.clicking"));
		(browser.getDriver()).getMouse().click(hoverItem.getCoordinates());
	}
